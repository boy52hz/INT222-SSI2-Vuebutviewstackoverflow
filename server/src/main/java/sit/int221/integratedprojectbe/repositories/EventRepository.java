...
public interface EventRepository extends JpaRepository<Event, Integer> {
        ...
        @Query(
                value = "select e.* from `event` e join `event_category_has_user` eco on e.categoryId = eco.categoryId  where eco.email =:email",
                nativeQuery = true
        )
        List<Event> findAllEventOfOwnerCategoryByEmail(@Param("email") String email);

        @Query(
                value = "select e.* from `event` e join `event_category_has_user` eco on e.categoryId = eco.categoryId where e.bookingId =:bookingId and eco.email =:email",
                nativeQuery = true
        )
